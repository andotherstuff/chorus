    ...
    queryFn: async (c) => {
      ...
      const relayPromises = groupRelays.map(async (relayUrl) => {
        try {
          ...
          const [groupEvents, memberEvents, adminEvents] = await Promise.all([
            ...
            // Admin list events
            nostr.query([{
              kinds: [39001], // NIP-29 relay-generated admin lists
              limit: 100
            }], { 
              signal,
              relays: [relayUrl] // Specify the relay explicitly
          ...
          console.log(`[NIP-29] Found ${groupEvents.length} group metadata events, ${memberEvents.length} member events, and ${adminEvents.length} admin events from ${relayUrl}`);
          if (groupEvents.length > 0) {
            console.log('[NIP-29] Sample group event:', groupEvents[0]);
          }

          // Create maps for member and admin lists
          ...
          for (const memberEvent of memberEvents) {
            ...
            if (dTag && dTag[1]) {
              ...
              memberMap.set(groupId, members);
            }
          }

          // Parse admin events  
          for (const adminEvent of adminEvents) {
            const dTag = adminEvent.tags.find(tag => tag[0] === 'd');
            if (dTag && dTag[1]) {
              const groupId = dTag[1];
              const admins: string[] = [];
              
              for (const tag of adminEvent.tags) {
                if (tag[0] === 'p' && tag[1] && tag[2] === 'admin') {
                  admins.push(tag[1]);
                }
              }
              
              adminMap.set(groupId, admins);
            }
          }
          ...
          for (const event of groupEvents) {
            ...
            if (group) {
              // Populate member and admin lists if available
              const members = memberMap.get(group.groupId) || [];
              const admins = adminMap.get(group.groupId) || [];
              
              group.members = members;
              group.admins = admins;
              console.log(`[NIP-29] Group ${group.name} has ${members.length} members and ${admins.length} admins`);
...
export function useNip29GroupMembers(groupId: string | undefined, relay: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: async (c) => {
      ...
      console.log(`[NIP-29] Found ${memberEvents.length} member events and ${adminEvents.length} admin events`);
