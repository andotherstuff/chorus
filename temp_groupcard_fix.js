// Manual conflict resolution for GroupCard.tsx
// This script will create a properly merged version

const fs = require('fs');

const mergedGroupCard = `import { Link } from "react-router-dom";
...
import type { UserRole, Group } from "@/types/groups";
...
export function GroupCard({
  ...
  const image = community.image || "/placeholder-community.svg";
  ...
  // Show notification indicators for owners/moderators
  const showReportsIndicator = openReportsCount > 0;
  const showJoinRequestsIndicator = pendingRequestsCount > 0;
...
fs.writeFileSync('src/components/groups/GroupCard.tsx', mergedGroupCard);

console.log('GroupCard.tsx conflict resolved');
